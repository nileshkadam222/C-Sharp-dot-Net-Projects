...
{
    public partial class frmJuniorReport : Form
    {
        connectionClass obj = new connectionClass();

        public frmJuniorReport()
        {
            InitializeComponent();
        }

        private void frmJuniorReport_Load(object sender, EventArgs e)
        {
            fillDepartment();
            showAll();
        }
        public void showAll()
        {
            try
            {
                ...
                da = new SqlDataAdapter("select [StudentId] as [Student ID],[StudentName] as [Student Name],[Department] as [Department],[Class] as [Class],[Address] as [Address],[Phone] as [Phone],[Email] as [Email] from tblJuniorStudent", obj.con);
                da.Fill(dt);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    ds.Tables.Add(dt);
                    SqlDataAdapter da = new SqlDataAdapter();
                    da = new SqlDataAdapter("select [StudentId] as [Student ID],[StudentName] as [Student Name],[Department] as [Department],[Class] as [Class],[Address] as [Address],[Phone] as [Phone],[Email] as [Email] from tblJuniorStudent where Department='" + cboDepartment.Text + "' and Class='" + cboClass.Text + "'", obj.con);
                    da.Fill(dt);
                    dataGridView1.DataSource = dt.DefaultView;
                }
                catch
